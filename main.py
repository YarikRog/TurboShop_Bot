...
async def update_cache_task():
    ...
    while True:
        ...
        except Exception as e: print(f"❌ Помилка кешу: {e}")
...
# ПІДБІР ПАРУ (Тимчасова заглушка до понеділка)
@dp.message_handler(lambda message: message.text == "🎯 Підібрати пару", state="*")
async def quiz_start(message: types.Message):
    await message.answer("Ця функція активується у понеділок після оновлення бази товарів! 🚀")
...
async def start_catalog(callback_query: types.CallbackQuery):
    ...
    if not products:
        await bot.answer_callback_query(callback_query.id, text="Немає в наявності.")
        return
...
async def show_product(user_id, index, message_to_edit=None):
    ...
    caption = f"👟 <b>{product.get('Бренд')} {product.get('Модель')}</b>\n💰 Ціна: <b>{product.get('Ціна')} грн</b>\n📏 Розмір: {data['size']}\n🆔 Артикул: <code>{product.get('Артикул')}</code>"
...
async def paginate(callback_query: types.CallbackQuery):
    ...
    await show_product(callback_query.from_user.id, new_index, callback_query.message.message_id)
...
async def process_buy(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(callback_query.from_user.id, "🚀 Поділіться номером телефону:", reply_markup=kb.get_contact_keyboard())

@dp.message_handler(content_types=['contact'], state=OrderState.waiting_for_phone)
async def get_phone(message: types.Message, state: FSMContext):
    await state.update_data(phone=message.contact.phone_number)
    await OrderState.next()
    await message.answer("✅ Тепер ПІБ отримувача:")
...
async def get_delivery(message: types.Message, state: FSMContext):
    ...
    try: requests.post(GAS_URL, json=data, timeout=10)
    except: pass
